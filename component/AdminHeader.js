...
export default function AdminHeader({ onMenuClick }) {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('❌ Failed to fetch user data:', response.status, errorData);
          
          if (response.status === 401) {
            // Unauthorized - likely no valid session
            setUser(null);
  ...
  console.log('🎨 Rendering header with:', { userName, userEmail, loading, error });
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4 lg:ml-0">
            <h2 className="text-lg font-semibold text-gray-900 hidden sm:block">
              {loading ? 'Loading...' : error ? 'Welcome back' : `Welcome back, ${userName}`}
            </h2>
            {error && (
